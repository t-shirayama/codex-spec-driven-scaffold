...
- [ ] 実装依頼前に `status: approved`、`blocking_open_questions: false` になっている
- [ ] Acceptance Criteriaの `Verified By` と `test-cases.md` の `Verified AC` が対応している
- [ ] `plan.md` に変更対象とリスクが書かれている
- [ ] `tasks.md` がCodexで実行できる粒度になっている
- [ ] `test-cases.md` に最低限の確認観点がある
- [ ] API変更がない場合も `api.md` に「API変更なし」と明記している
